...
class SpyNet(nn.Module):
    ...
    def preprocess(self, tensor_input):
        # Cast mean/std to match input dtype (handles FP16)
        mean = self.mean.to(tensor_input.dtype)
        std = self.std.to(tensor_input.dtype)
        tensor_output = (tensor_input - mean) / std
...
@lru_cache()
def compute_mask(D, H, W, window_size, shift_size, device, dtype=torch.float32):
...
class WindowAttention(nn.Module):
    ...
    def attention(self, q, k, v, mask, x_shape):
        ...
        # Expand bias for all batches
        attn_bias = relative_position_bias.expand(B_, self.num_heads, N, N)

        # Add window mask if provided
        if mask is not None:
            nW = mask.shape[0]
            attn_bias = attn_bias.view(B_ // nW, nW, self.num_heads, N, N)
            attn_bias = attn_bias + mask[:, :N, :N].unsqueeze(1).unsqueeze(0)
            attn_bias = attn_bias.view(-1, self.num_heads, N, N)

        # Use PyTorch's native scaled_dot_product_attention which includes Flash Attention
        # Note: Flash Attention doesn't support arbitrary additive bias (like relative position bias)
        # so we use the manual implementation with bias and optimized matmuls
        # PyTorch 2.0+ automatically selects efficient kernels (Flash Attention, memory-efficient, or math)
        # when using scaled_dot_product_attention, but RVRT's relative position bias requires manual computation

        # Compute attention scores with scale
        attn = (q * self.scale) @ k.transpose(-2, -1)
        attn = attn + attn_bias
        attn = F.softmax(attn, -1, dtype=q.dtype)
...
class RVRT(nn.Module):
    ...
    Args:
        ...
        cpu_cache_length: (int): Maximum video length without cpu caching. Default: 128.
    """
