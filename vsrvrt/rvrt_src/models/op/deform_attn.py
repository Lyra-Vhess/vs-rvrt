...
#
# Modified for vsrvrt: Uses pre-built binaries instead of JIT compilation.
...
def _load_deform_attn_extension():
    """Load the pre-built deform_attn CUDA extension.
    
    This function loads the appropriate pre-compiled binary based on
    platform and Python version. No JIT compilation is performed.
    
    Returns:
        module: The loaded deform_attn extension module
    """
    from vsrvrt._binary import load_deform_attn
    return load_deform_attn()


def _ensure_extension_loaded():
    """Ensure the extension is loaded, raising a clear error if not."""
    global deform_attn_ext
    if deform_attn_ext is None:
        deform_attn_ext = _load_deform_attn_extension()
    return deform_attn_ext
